...
impl DrawFn for GearboxAllDrawer {
    ...
    fn draw_apply<'a, DB: DrawingBackend + 'a>(
        ...
        for (graph, chart) in data.iter().zip(charts.iter()) {
            ...
            let bottle_trace = &result.total_result.loc_acc_bottleneck;
            ...
            bank_chart.draw_series(
                bank_trace
                    .iter()
                    .zip(bottle_trace.iter())
                    .enumerate()
                    .map(|(i, (x, bottle))| {
                        (
                            (i as f32 / data_len as f32, *x as f32 / *bank_max as f32),
                            bottle,
                        )
                    })
                    .map(|(coord, bottle)| match bottle {
                        analyze_refined_bank_trace::Bottleneck::Local => {
                            Circle::new(coord, 1, RED.filled())
                        }
                        analyze_refined_bank_trace::Bottleneck::Dispatcher => {
                            Circle::new(coord, 1, BLACK.filled())
                        }
                    }),
            )?;

            local_chart.draw_series(
                sub_local
                    .iter()
                    .zip(bottle_trace.iter())
                    .enumerate()
