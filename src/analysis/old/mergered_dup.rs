...
enum EventType {
    SubarrayFinished(usize),
    Started,
}

type Event = super::event::Event<EventType>;
...
impl<LevelType: LevelTrait> ResourceConstraint<LevelType> {
    ...
    fn allocate(
        ...
        current_time: u64,
    ) -> Result<Vec<Event>, RoundTasks<LevelType>> {
        let mut events = vec![];
    ...
    fn deallocate(&mut self, sub_array_id: usize) {
    ...
    LevelType::Storage: Ord,
{
    ...
    for graph in &config.graph_path {
        ...
        let mut event_queue: BinaryHeap<Reverse<Event>> = BinaryHeap::new();
