...
struct SubArray {
    ...
    local_read_rows_evil: Vec<(PhysicRowId, usize)>,
    local_write_rows_evil: Vec<(PhysicRowId, usize)>,

    remote_read_rows: Vec<(PhysicRowId, usize)>,
    remote_write_rows: Vec<(PhysicRowId, usize)>,
}
...
impl SubArray {
    ...
    fn new() -> Self {
        Self {
            ...
            local_read_rows_evil: Vec::new(),
            local_write_rows_evil: Vec::new(),
            remote_read_rows: Vec::new(),
            remote_write_rows: Vec::new(),
        }
    }
    ...
    fn add_task(&mut self, local_read: PhysicRowId, local_write: PhysicRowId, evil: bool) {
        if evil {
            match self.local_read_rows_evil.last() {
                Some((last_read, _)) if *last_read == local_read => {
                    self.local_read_rows_evil.last_mut().unwrap().1 += 1;
                }
                _ => {
                    self.local_read_rows_evil.push((local_read, 1));
                }
            }
            match self.local_write_rows_evil.last() {
                Some((last_write, _)) if *last_write == local_write => {
                    self.local_write_rows_evil.last_mut().unwrap().1 += 1;
                }
                _ => {
                    self.local_write_rows_evil.push((local_write, 1));
                }
            }
        } else {
    ...
    fn report(&self) -> SubArrayResult {
        ...
        // for evil row(local)
        let mut row_open_cycle_evil = 0;
        let mut row_read_cycle_evil = 0;
        let mut row_write_cycle_evil = 0;
        for (row_id, read_times) in self.local_read_rows_evil.iter() {
            match self.read_open {
                Some(row) => {
                    if row == row_id.0 {
                    } else {
                        cycle += 18;
                        row_open_cycle_evil += 18;
                        row_read_cycle_evil += 18;
                    }
                }
                None => {
                    cycle += 9;
                    row_open_cycle_evil += 9;
                    row_read_cycle_evil += 9;
                }
            }
            cycle += read_times;
            comp_cycle += read_times;
        }
        for (row_id, read_times) in self.local_write_rows_evil.iter() {
            match self.write_open {
                Some(row) => {
                    if row == row_id.0 {
                    } else {
                        cycle += 18;
                        row_open_cycle_evil += 18;
                        row_write_cycle_evil += 18;
                    }
                }
                None => {
                    cycle += 9;
                    row_open_cycle_evil += 9;
                    row_write_cycle_evil += 9;
                }
            }
            cycle += read_times;
        }
        // for remote tasks
        let mut remote_acc_cycle = 0;
        let mut row_read_cycle_remote = 0;
        let mut row_write_cycle_remote = 0;
        for (row_id, read_times) in self.remote_read_rows.iter() {
            match self.read_open {
                Some(row) => {
                    if row == row_id.0 {
                    } else {
                        cycle += 18;
                        row_read_cycle_remote += 18;
                    }
                }
                None => {
                    cycle += 9;
                    row_read_cycle_remote += 9;
                }
            }
            cycle += read_times;
            comp_cycle += read_times;
        }
        // the remote write will be counted as remote acc cycle
        for (row_id, read_times) in self.remote_write_rows.iter() {
            match self.write_open {
                Some(row) => {
                    if row == row_id.0 {
                    } else {
                        remote_acc_cycle += 18;
                        row_write_cycle_remote += 18;
                    }
                }
                None => {
                    remote_acc_cycle += 9;
                    row_write_cycle_remote += 9;
                }
            }
            remote_acc_cycle += read_times;
        }

        SubArrayResult {
            cycle,
            local_row_open_cycle: row_open_cycle,
            local_row_read_cycle: row_read_cycle,
            local_row_write_cycle: row_write_cycle,
            comp_cycle,
            local_row_open_cycle_evil: row_open_cycle_evil,
            local_row_read_cycle_evil: row_read_cycle_evil,
            local_row_write_cycle_evil: row_write_cycle_evil,
            remote_row_read_cycle: row_read_cycle_remote,
            remote_row_write_cycle: row_write_cycle_remote,
            cycle_remote: remote_acc_cycle,
        }
    }
    ...
    fn add_remote_read_task(&mut self, local_read: PhysicRowId) {
        match self.remote_read_rows.last() {
            Some((last_read, _)) if *last_read == local_read => {
                self.remote_read_rows.last_mut().unwrap().1 += 1;
            }
            _ => {
                self.remote_read_rows.push((local_read, 1));
            }
        }
    }
    ...
    fn add_remote_task(&mut self, local_write: PhysicRowId) {
        match self.remote_write_rows.last() {
            Some((last_write, _)) if *last_write == local_write => {
                self.remote_write_rows.last_mut().unwrap().1 += 1;
            }
            _ => {
                self.remote_write_rows.push((local_write, 1));
            }
        }
    }
}
...
impl GearboxSim {
    ...
    fn run(&mut self, input_vec: &CsVec<Pattern>) {
        ...
        // distribute the task to components
        for mat_b_row_id in input_vec.indices() {
            if self.evil_row_ids.contains(mat_b_row_id) {
                ...
                self.hardware
                    .distribute_evil_col(LogicRowId(0), evil_col_row_id_col_id);
            }
        }
    }
...
fn compute_gearbox(config: &Config, path: &str) -> SingleResult {
    ...
    gearbox.run(&matrix_a.outer_view(0).unwrap().to_owned());
