        ...
        for (graph, chart) in data.iter().zip(charts.iter()) {
            ...
            for (i, &TopK(topk)) in topks.iter().enumerate() {
                for (j, &batch) in batches.iter().enumerate() {
                    ...
                    let local_rate = local as f32 / total as f32;
                    let real_local_rate = real_local as f32 / total as f32;
                    let send_remote_rate = send_remote as f32 / total as f32;
                    let dispatching_rate = dispatching as f32 / total as f32 + real_local_rate;
                    let remote_rate = remote as f32 / total as f32 + dispatching_rate;
