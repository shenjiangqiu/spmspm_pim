...
//     fn add_task(
//         &mut self,
//         local_read_row: PhysicRowId,
//         local_read_col: PhysicColId,
...
//                 Some((last_read_row, last_read_col)) => {
//                     if last_read_row == local_read_row.0 {
...
//                         self.read_open = Some((local_read_row.0, local_read_col.0));
//                     }
//                 }
//                 _ => {
...
//                         let write_cycle = if last_write_col > col_id.0 {
//                             last_write_col - col_id.0
//                         } else {
//                             col_id.0 - last_write_col
//                         };
//                         self.sub_array_result.local_row_write_cycle_evil += write_cycle;
//                         self.sub_array_result.cycle += write_cycle;
//                         self.write_open = Some((last_write_row, col_id.0));
//                     } else {
//                         self.sub_array_result.local_row_open_cycle_evil += 19;
//                         self.sub_array_result.cycle += 19;
//                         self.write_open = Some((local_write.0, col_id.0));
//                     }
//                 }
//                 _ => {
//                     self.sub_array_result.local_row_open_cycle_evil += 9;
...
//     fn add_remote_read_task(&mut self, local_read: PhysicRowId, local_read_col: PhysicColId) {
//         match self.read_open {
//             Some((last_read_row, last_read_col)) => {
//                 if last_read_row == local_read.0 {
//                     self.sub_array_result.remote_row_read_cycle += 1;
//                     self.sub_array_result.cycle += 1;
//                 } else {
//                     self.sub_array_result.remote_row_read_cycle += 19;
//                     self.sub_array_result.cycle += 19;
//                     self.read_open = Some((local_read.0, local_read_col.0));
//                 }
//             }
//             _ => {
...
//     fn add_remote_task(&mut self, local_write: PhysicRowId, col_id: PhysicColId) {
//         match self.remote_write {
//             Some((last_write_row, last_write_col)) => {
//                 if last_write_row == local_write.0 {
//                     let shift_cycle = if col_id.0 > last_write_col {
//                         col_id.0 - last_write_col
//                     } else {
//                         last_write_col - col_id.0
//                     };
...
//                     self.remote_write = Some((local_write.0, col_id.0));
//                 } else {
...
//         mut col_ids: Vec<(SubarrayId, LogicColId)>,
//     ) {
//         col_ids.sort_unstable_by_key(|x| x.0);
//         let subarray_tasks: HashMap<_, Vec<_>> =
//             col_ids.into_iter().into_grouping_map_by(|x| x.0).collect();
//         for (subarray_id, col_ids) in subarray_tasks {
//             self.read_local(
//                 target_row_id,
//                 subarray_id,
//                 physic_row_id,
...
//                 lenth,
//             );
//             self.mapping.get_matrix_b_location_evil(mat_b_row_id)
//             for (_, col_id) in col_ids {
//                 let (_subarray_id, physic_row_id, physic_col_id) = self
//                     .mapping
//                     .get_result_dense_location(target_row_id, col_id);
//                 assert_eq!(subarray_id, _subarray_id);

//                 self.write_dense_local(target_row_id, subarray_id, physic_row_id, physic_col_id);
//             }
//         }
//     }
...
//         &mut self,
//         target_row_id: LogicRowId,
//         row_id_col_id: BTreeMap<SubarrayId, BTreeMap<LogicColId, Vec<(LogicRowId, usize)>>>,
//     ) {
...
//         for (source_subarray_id, tasks) in row_id_col_id {
//             // step 1: read local
//             for task in tasks {
//                 let task_size = task.1.len();
//                 let target_subarray_id = self
//                     .mapping
//                     .get_result_dense_location(target_row_id, task.0)
//                     .0;
//                 if source_subarray_id == target_subarray_id {
//                     self.distribute_local_write_dense(target_subarray_id, target_row_id, task.0)
//                 } else {
//                     self.distribute_remote(
//                         target_row_id,
//                         matrix_b_row_id,
//                         col_id,
//                         shift,
...
//                 }
//             }
//             // // setp 2: write to remote

//             // // noticed here, the evil col
//             // let partition_id = self.mapping.get_matrix_b_location(mat_b_row_id).0;
//             // let target_partition_id = self.mapping.get_result_dense_location(0.into(), col_id).0;
//             // if partition_id == target_partition_id {
//             //     self.distribute_local(target_row_id, mat_b_row_id, col_id, shift);
//             // } else {
//             //     // record the remote tasks
//             //     // first read the local row
//             //     let physic_row_id = self
//             //         .mapping
//             //         .get_matrix_b_location_with_shift(mat_b_row_id, shift);
//             //     self.sub_array[partition_id.0]
//             //         .add_remote_read_task(physic_row_id.1, physic_row_id.2);
...
//     ) {
//         // read the local row
//         let local_read_row_id = self
//             .mapping
//             .get_matrix_b_location_with_shift(matrix_b_row_id, shift);
//         self.sub_array[local_subarray.0]
//             .add_remote_read_task(local_read_row_id.1, local_read_row_id.2);
//         todo!()
//         // self.distribute_remote(target_row_id, subarray_id, col_id);
//     }

//     fn ring_buffer_id(&self, subarray_id: SubarrayId) -> RingBufferId {
//         let subarray_id = subarray_id.0;
//         let ring_buffer_id = subarray_id / self.config.subarrays;
//         RingBufferId(ring_buffer_id)
//     }
...
//         shift: usize,
//         len: usize,
//         target_subarray_id: SubarrayId,
//     ) {
//         // ignore the ring and tsv here cause it's overlaped with local accumulation.
//         let target_partition_id = self
//             .mapping
//             .get_result_dense_location(target_row_id, col_id);
//         self.sub_array[target_partition_id.0 .0]
//             .add_remote_task(target_partition_id.1, target_partition_id.2);
//         let target_bank_id = self.ring_buffer_id(target_partition_id.0);
//         todo!()
...
//     fn distribute_local_write_dense(
//         &self,
//         target_subarray_id: SubarrayId,
...
//         let subarray_bits = tools::math::count_to_log(config.subarrays);
//         let bank_bits = tools::math::count_to_log(config.banks.num);
//         Self {
...
//         _current_topk: f32,
//     ) -> GlobalStatV2 {
//         // file stream to write the bank trace,
...
//             // print the progress each TIME_TO_LOG seconds
...
//             // the evil tasks for this round, it includes(row id, col id, col id index in the row)
//             let mut evil_col_row_id_col_id: BTreeMap<
//                 SubarrayId,
//                 BTreeMap<LogicColId, Vec<(LogicRowId, usize)>>,
//             > = BTreeMap::new();
...
//                     );
//                 } else {
//                     // the source subarray id
//                     let (source_subarray_id, physic_row_id, physic_col_id) = self
//                         .hardware
//                         .mapping
//                         .get_matrix_b_location(LogicRowId(mat_b_row_id));
//                     let row_size = self.matrix_b.outer_view(mat_b_row_id).unwrap().nnz();
//                     // the row is not evil, need to access remote
//                     for (shift, col) in self
//                         .matrix_b
//                         .outer_view(mat_b_row_id)
//                         .unwrap()
//                         .indices()
...
//                                 .entry(local_subarray_id.0)
//                                 .or_default()
//                                 .entry(LogicColId(col))
//                                 .or_default()
//                                 .push((LogicRowId(mat_b_row_id), shift));
//                         }
//                     }

//                     // get the all partitions that this row will be distributed to

//                     // the col is not evil, need to access remote
//                     // it contains, the start col_id, the col_id shift, the col_id length.
//                     let mut all_tasks: BTreeMap<SubarrayId, (LogicColId, usize, usize)> =
//                         BTreeMap::new();
//                     let mut local_tasks: Option<(LogicColId, usize, usize)> = None;
...
//                             .hardware
//                             .mapping
//                             .get_result_dense_location(LogicRowId(target_id), LogicColId(col));
//                         if target_partition == local_subarray_id {
//                             if let Some((start, shift, len)) = local_tasks {
//                                 local_tasks = Some((start, shift, len + 1));
//                             } else {
//                                 local_tasks = Some((LogicColId(col), shift, 1));
//                             }
//                         } else {
//                             if let Some((start, shift, len)) =
//                                 all_tasks.get_mut(&target_partition.0)
//                             {
//                                 *len += 1;
//                             } else {
//                                 all_tasks.insert(target_partition.0, (LogicColId(col), shift, 1));
//                             }
//                         }
//                     }
//                     // distribute the tasks collected before
//                     if let Some((start, shift, len)) = local_tasks {
...
//                         );
//                     }
//                     for (subarray_id, (start, shift, len)) in all_tasks {
...
//     #[test]
//     fn test() {
//         let a = P::P(NonZeroU32::new(1).unwrap());
//         let a = match a {
//             P::P(a) => a,
//             P::N => panic!(""),
//         };
