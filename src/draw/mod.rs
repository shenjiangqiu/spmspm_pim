...
use spmspm_pim::{
    analysis::{analyze_gearbox, analyze_gearbox_origin, analyze_gearbox_origin_all},
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.subcmd {
        ...
        spmspm_pim::cli::DrawType::GearBoxAll(gearbox_result) => draw_gearbox_all(gearbox_result)?,
    }
    Ok(())
}
...
/// draw gearbox cycle distribution
fn draw_gearbox_all(gearbox_args: ExecResult) -> Result<(), Box<dyn Error>> {
    let ExecResult {
        result_file,
        output,
    } = gearbox_args;
