...
use super::DrawFn;
use crate::{
    analysis::analyze_gearbox_origin_all_v2_overflow, cli::ExecResult, draw::draw_data_with_size,
};
...
pub fn draw(result: ExecResult) -> eyre::Result<()> {
    ...
    let results: Vec<_> = files
        ...
        .map(|(batch, tokp, file_name)| -> eyre::Result<_> {
            ...
            let result: Vec<analyze_gearbox_origin_all_v2_overflow::SingleResult> =
    ...
    let transposed_result = analyze_gearbox_origin_all_v2_overflow::transpose2(results);
    info!(
...
impl DrawFn for GearboxAllDrawer {
    type DATA = Vec<Vec<analyze_gearbox_origin_all_v2_overflow::SingleResult>>;
            ...
            for (i, &TopK(topk)) in topks.iter().enumerate() {
                for (j, &batch) in batches.iter().enumerate() {
                    ...
                    let overflow_text = format!(
                        "overflow: 8:256:({}), \n8:512:({}),\n12:256:({}),\n12:512:({})",
                        result.total_result.overflow_count_8_256,
                        result.total_result.overflow_count_8_512,
                        result.total_result.overflow_count_12_256,
                        result.total_result.overflow_count_12_512
                    );
                    chart.draw_series([Text::new(
                        overflow_text,
                        (i as f32, max_cycle, j as f32),
                        ("sans-serif", 10).into_font(),
                    )])?;
                }
            }
