...
//!  w
use crate::{
    analysis::{remap_analyze::row_cycle::*, RingTask, RingTasksInAllBanks},
    tools::{
...
impl SubArray {
    ...
    fn add_remote_read_task(&mut self, local_read: PhysicRowId) {
        match self.read_open {
            Some(last_read) => {
                if last_read == local_read.0 {
                    // because the local index read is always continuous, so the cycle is 1
                    self.sub_array_result.remote_row_read_cycle += 1;
                    self.sub_array_result.cycle += 1;
                } else {
                    // remote row open cycle
...
impl Hardware {
    ...
    fn get_tsv_interleave(&self) -> Vec<VecDeque<&RingTask>> {
        ...
        tsv_traffic.map(|t| t.collect()).collect()
    }
...
// the dispatcher received packets to cycles
fn packets_to_cycle(packets: usize, p_type: PackType) -> usize {
    // for each row, there should be a precharge and activate for read
    // there should be a precharge for write
    // there are 256 bytes in a row
    // there are 8 bytes in a packet
    // precharge and activate are 9 cycles
    // if it's less than a row, do not need to precharge and activate
    match p_type {
        PackType::Read => {
            let row = packets / 32;
            let precharge = row * 9;
...
fn update_stats(hardware: &mut Hardware, global_stats: &mut GlobalStatV2) {
    ...
    let ring_buffer_max = hardware.ring_buffer.iter_mut().map(|ring_buffer| {
        ...
        ring_buffer.report_and_reset()
    });
    // the local accumulate stage dispacher cycle, it include write local package and write remote packages, only the received package will remain in dispatcher

    let (max_local_dispatcher, max_dispatching) = ring_buffer_max
        .map(|(sent, received)| {
            let local_cycle = packets_to_cycle(sent, PackType::Write)
                + packets_to_cycle(received, PackType::Write);
            let dispatching_cycle = packets_to_cycle(received, PackType::Read);
            (local_cycle, dispatching_cycle)
        })
        .fold((0, 0), |(local_acc, dispatching), (c_l, c_d)| {
            (local_acc.max(c_l), dispatching.max(c_d))
        });
    ...
    let max_real_local_cycle = max_local_cycle.max(&max_local_dispatcher);
    ...
    global_stats.global_max_ring_buffer += max_local_dispatcher;
    global_stats.global_max_dispatching += max_dispatching;
}
