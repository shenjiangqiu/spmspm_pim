...
pub fn draw(result: ExecResult) -> eyre::Result<()> {
    ...
    let results: Vec<_> = files
        ...
        .map(|(batch, tokp, file_name)| -> eyre::Result<_> {
            ...
            let result: Vec<analyze_refined_dispatcher_overflow::SingleResult> =
    ...
    let transposed_result = analyze_refined_dispatcher_overflow::transpose2(results);
    info!(
...
impl DrawFn for GearboxAllDrawer {
    type DATA = Vec<Vec<analyze_refined_dispatcher_overflow::SingleResult>>;
