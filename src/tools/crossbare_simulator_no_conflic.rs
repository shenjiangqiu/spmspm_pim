//! the simulator for crossbare without conflict

use std::collections::VecDeque;

use super::CrossBarPacket;

pub struct CrossBareSimulatorNoConflict<T> {
    ports: usize,
    buffer_size: usize,
...
impl<T> CrossBareSimulatorNoConflict<T> {
...
impl<T: CrossBarPacket> CrossBareSimulatorNoConflict<T> {
    pub fn cycle(&mut self) {
        for i in 0..self.ports {
            ...
            if let Some(traffic) = self.input_buffer[index].pop_front() {
                ...
                if self.output_buffer[target].len() >= self.buffer_size {
...
mod tests {
    ...
    #[test]
    fn test1() {
        let mut sim = CrossBareSimulatorNoConflict::new(4, 2);
