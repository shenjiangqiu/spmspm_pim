...
impl Hardware {
    ...
    /// get the traffic for each tsv channel
    fn calculate_channel(&self) -> [Vec<usize>; 2] {
        let mut output_traffic = vec![0; self.config.channels.num];
        let input_traffic = self
            .ring
            .iter()
            .enumerate()
            .map(|(rid, r)| {
                r.tasks
                    ...
                    .flatten()
                    .flatten()
                    .filter(|data| rid != data.2 .0 .0)
                    .map(|data| {
                        let target_channel = data.2 .0 .0;
                        output_traffic[target_channel] += 1;
                    })
                    .count()
            })
            .collect_vec();
    ...
    where
        Cross: CrossBarCommon<CrossBarTraffic<&'a RingTask>>,
    {
...
pub struct GlobalStatV2 {
    pub channel_stats_input: Vec<Vec<usize>>,
    pub channel_stats_output: Vec<Vec<usize>>,
}

fn update_stats(hardware: &mut Hardware, global_stats: &mut GlobalStatV2) {
    // test if the size is overflow!
    let [input, output] = hardware.calculate_channel();

    hardware.ring.iter_mut().for_each(|ring| {
        ring.report_current_round();
    });
    hardware.tsv.iter_mut().for_each(|tsv| {
        tsv.report_current_round();
    });
    // get the max ring buffer cycle and count for the overflow
    hardware.ring_buffer.iter_mut().for_each(|ring_buffer| {
    ...
    hardware.sub_array.iter_mut().for_each(|sub_array| {
        sub_array.report_current_round();
    });

    global_stats.channel_stats_input.push(input);
    global_stats.channel_stats_output.push(output);
}
...
fn compute_gearbox(config: &ConfigV2, path: &str) -> Vec<SingleResult> {
    ...
    let mut _guard = crate::acquire_memory_sections([sim_size as u64, temp_size as u64]);
