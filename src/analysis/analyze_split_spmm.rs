...
use super::split::{split_matrix, NnzStats};
    ...
    LevelType::Mapping: Debug,
{
    let results = config
        ...
        .map(|path| {
            ...
            for (_bank_id, single_matrix) in s_matrix.matrix.iter().enumerate() {
...
fn sparse_add(v1: CsVecView<Pattern>, v2: CsVecView<Pattern>) -> CsVec<Pattern> {
    ...
    while v1_next.is_some() || v2_next.is_some() {
        match (v1_next, v2_next) {
            (Some((i1, _)), Some((i2, _))) => match i1.cmp(&i2) {
    ...
    LevelType::Mapping: Debug,
{
    ...
    for task in matrix_a.outer_iterator() {
        ...
        for (task_id_b, _) in task.iter() {
            ...
            let (temp_result1, temp_result2, opened_rows) = current_temp.open_row(
                (0, 0),
            ...
            let (final_result1, final_result2, opened_rows) = current_final.open_row(
                (0, 0),
                current_result.nnz() * 4,
                config.activate_cycle as usize,
                config.precharge_cycle as usize,
