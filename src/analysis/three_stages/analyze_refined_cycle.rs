...
impl<'a, MP: Mapping> Hardware<'a, MP> {
    ...
    fn distribute_evil_col(
        &mut self,
        target_row_id: LogicRowId,
        row_id_col_id: impl IntoIterator<Item = (LogicRowId, LogicColId, usize)>,
    ) {
        ...
        for (mat_b_row_id, col_id, shift) in row_id_col_id {
...
impl<'a, 'b, MP: Mapping> GearboxSim<'a, 'b, MP> {
    ...
    fn run(
        ...
        for (target_id, row) in input_vec.outer_iterator().enumerate() {
            ...
            let mut evil_col_row_id_col_id: Vec<(LogicRowId, LogicColId, usize)> = vec![];
                        ...
                        .map(|(shift, i)| (shift, *i as usize))
                    {
                        if self.evil_col_ids.contains(&col) {
                            ...
                            if target_partition == source_partition {
                                ...
                                let source =
                                    self.hardware.mapping.get_matrix_b_location_with_shift(
                                        LogicRowId(mat_b_row_id),
                                        shift,
                                    );
                                let dst = self.hardware.mapping.get_result_dense_location(
                                    LogicRowId(target_id),
                                    LogicColId(col),
                                );
...
impl Display for GlobalStatV2 {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "local: max: {} mean: {} dist: max: {} mean: {} dispatch: max: {} mean: {} remote: max: {} mean: {} real: max: {}",
